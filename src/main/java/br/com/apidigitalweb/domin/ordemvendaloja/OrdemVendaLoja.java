...
@Entity
@NoArgsConstructor
public class OrdemVendaLoja  extends BaseOrdem implements  Serializable {

	private static final long serialVersionUID = 1L;
	@JsonIgnore
	@JsonIgnoreProperties({ "hibernateLazyInitializer", "handler" })
	@OneToMany(mappedBy = "ordem", fetch = FetchType.LAZY, cascade = CascadeType.ALL)
	private List<ItensMaterialInVendaLoja> itensMaterialInVendaLojas;
	...
	public double getTotalItensMaterial() {
		...
		try {
			...
			for (ItensMaterialInVendaLoja itensInsService : getItensMaterialInVendaLojas()) {
