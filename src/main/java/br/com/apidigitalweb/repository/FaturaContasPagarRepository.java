...
public interface FaturaContasPagarRepository extends JpaRepository<FaturaContasPagar, Long> {
	...
	@Query("SELECT SUM( e.valor - e.desconto + e.jurus + e.multa) from FaturaContasPagar e where e.status=?1")
	double totalAll(String status);

	@Query("SELECT new br.com.apidigitalweb.dto.financeiro.FaturasDto(f) from FaturaContasPagar f where f.status=?1 order by f.dataVencimento")
