...
public interface ContasPagarRepository extends JpaRepository<ContasPagar, Long> {
	@Query("select distinct  e.grupoFinanceiro from ContasPagar e  where e.status=?1")
	List<GrupoContaPagar> findAllByGrupoContaPagar(String Status);
	...
	@Query("select distinct  e from ContasPagar e  " + "where e.status=?1")
